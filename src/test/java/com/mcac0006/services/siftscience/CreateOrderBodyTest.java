...
public class CreateOrderBodyTest {
	...
	public void createOrderTest() throws JsonGenerationException, JsonMappingException, IOException {
			...
			final String json = SiftScienceHelper.DEFAULT.serialize(create_order); // the json object we will be asserting
			...
			this.assertEquals(pmj.keySet(), pmexpectedKeys);
			...
			this.assertEquals(ba.keySet(), baexpectedKeys);
			...
			this.assertEquals(i1.keySet(), iexpectedKeys);
			...
			this.assertEquals(i2.keySet(), i2expectedKeys);
			...
			this.assertEquals(i3.keySet(), i3expectedKeys);
